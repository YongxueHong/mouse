...
def run_case(params):
    ...
    test = CreateTest(case_id='rhel7_58672', params=params)
    id = test.get_id()
    ...
    test.main_step_log('7.during migration in progress, '
                       'use firewall on destination host')
    status = utils_migration.query_status(remote_qmp=src_remote_qmp,
                                          status='active')
    if (status == False):
        test.test_error('Migration status is not active')
    cmd = 'ssh root@%s iptables -F' % dst_host_ip
    src_host_session.host_cmd(cmd=cmd)
    cmd = 'ssh root@%s iptables -A INPUT -p tcp --dport %s -j REJECT' \
          % (dst_host_ip, incoming_port)
    src_host_session.host_cmd(cmd=cmd)
    cmd = 'ssh root@%s iptables -L | grep REJECT' % dst_host_ip
    output = src_host_session.host_cmd_output(cmd=cmd)
    if not output:
        test.test_error('Failed to set iptables')

    test.main_step_log('8. Cancel migration immediately after step 6')
    status = utils_migration.query_status(remote_qmp=src_remote_qmp,
                                          status='active')
    if (status == False):
        test.test_error('Migration status is not active')
    ...
    output = src_host_session.host_cmd_output(cmd=cmd)
    if output:
        test.test_error('Failed to flush iptables')