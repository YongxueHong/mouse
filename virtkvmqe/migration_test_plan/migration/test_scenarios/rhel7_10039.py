...
def run_case(params):
    ...
    output = src_remote_qmp.qmp_cmd_output('{"execute":"query-block"}', recv_timeout=5)
    ...
    src_host_session.host_cmd_output('qemu-img create -f qcow2 %s/data-disk0.qcow2 10G' %share_images_dir)
    src_host_session.host_cmd_output('qemu-img create -f qcow2 %s/data-disk1.qcow2 20G' %share_images_dir)
    ...
    src_remote_qmp.qmp_cmd_output('{"execute":"__com.redhat_drive_add", "arguments":'
                           ...
                           '"format":"qcow2","id":"drive-virtio-blk0"}}' %share_images_dir, recv_timeout=5)
    src_remote_qmp.qmp_cmd_output('{"execute":"device_add","arguments":{"driver":"virtio-blk-pci","drive":"drive-virtio-blk0",'
                           '"id":"virtio-blk0","bus":"pci.0","addr":"10"}}', recv_timeout=5)
    src_remote_qmp.qmp_cmd_output('{"execute":"__com.redhat_drive_add", "arguments":'
                           '{"file":"/%s/data-disk1.qcow2","format":"qcow2","id":"drive_r4"}}' %share_images_dir, recv_timeout=5)
    src_remote_qmp.qmp_cmd_output('{"execute":"device_add","arguments":{"driver":"scsi-hd",'
                           '"drive":"drive_r4","id":"r4","bus":"virtio_scsi_pci0.0","channel":"0","scsi-id":"0","lun":"1"}}', recv_timeout=5)
    ...
    params.vm_base_cmd_add('drive', 'file=/%s/data-disk0.qcow2,format=qcow2,if=none,id=drive-virtio-blk0,werror=stop,rerror=stop'
                           %share_images_dir)
    params.vm_base_cmd_add('device', 'virtio-blk-pci,drive=drive-virtio-blk0,id=virtio-blk0,bus=pci.0,addr=10,bootindex=10')
    params.vm_base_cmd_add('drive', 'file=/%s/data-disk1.qcow2,if=none,id=drive_r4,format=qcow2,cache=none,aio=native,werror=stop,rerror=stop'
                           %share_images_dir)
